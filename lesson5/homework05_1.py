...
year_min, engine_volume_min, price_max = search_criteria #разпаковуємо критерії в окремі зміні

#Через цикл фільтуємо машини по заданим критеріям
filtered_cars = [
    (car, char) for car, char in car_data.items()
    if char[1] >= year_min and char[2] >= engine_volume_min and char[4] <= price_max
]
print(filtered_cars)
filtered_cars.sort(key=lambda filtered_cars: filtered_cars[1][4])
